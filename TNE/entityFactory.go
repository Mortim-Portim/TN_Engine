...
func (cf *EntityFactory) LoadEntityFromCreationData(data []byte) (*Entity, error) {
	...
	e.SetMaxHealth(float64(vals[0]))
	e.SetMaxStamina(float64(vals[1]))
	e.SetMaxMana(float64(vals[2]))
	e.SetHealth(float64(vals[3]))
	e.SetStamina(float64(vals[4]))
	e.SetMana(float64(vals[5]))
