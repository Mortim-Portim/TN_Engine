...
func (d *Direction) FromID() {
	if d.ID == ENTITY_ORIENTATION_LU {
		d.L = true; d.U = true; d.R = false; d.D = false
		d.Dx = -ABForH1; d.Dy = -ABForH1
	}else if d.ID == ENTITY_ORIENTATION_RD {
		d.L = false; d.U = false; d.R = true; d.D = true
		d.Dx = ABForH1; d.Dy = ABForH1
	}else if d.ID == ENTITY_ORIENTATION_RU {
		d.L = false; d.U = true; d.R = true; d.D = false
		d.Dx = ABForH1; d.Dy = -ABForH1
	}else if d.ID == ENTITY_ORIENTATION_LD {
		d.L = true; d.U = false; d.R = false; d.D = true
		d.Dx = -ABForH1; d.Dy = ABForH1
	}else if d.ID == ENTITY_ORIENTATION_L {
		d.L = true; d.U = false; d.R = false; d.D = false
		d.Dx = -1; d.Dy = 0
	}else if d.ID == ENTITY_ORIENTATION_R {
		d.L = false; d.U = false; d.R = true; d.D = false
		d.Dx = 1; d.Dy = 0
	}else if d.ID == ENTITY_ORIENTATION_U {
		d.L = false; d.U = true; d.R = false; d.D = false
		d.Dx = 0; d.Dy = -1
	}else if d.ID == ENTITY_ORIENTATION_D {
		d.L = false; d.U = false; d.R = false; d.D = true
		d.Dx = 0; d.Dy = 1
	}else{
		d.ID = INVALID_DIR
	}
}
func (d *Direction) FromKeys() {
	if d.L && d.R || d.U && d.D {
		d.ID = INVALID_DIR
	}else if d.L && d.U {
